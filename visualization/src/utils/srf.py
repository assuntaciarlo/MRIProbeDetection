...
def read_srf2(filename):
    ...
    data : dictionary
        ...
        "vertex color indices" : 1D numpy.array, (nr_vertices,)
            Raw color index per vertex, as stored in the file (int32). This
            mirrors BrainVoyager's own "SurfaceColor" array; it is not yet
            decoded into an actual color. See BrainVoyager's Users Guide 2.3
            for the full index scheme (curvature colors, POI colors 10000-
            10200, functional LUT colors >=1000, direct RGB >=1056964608).
        "vertex colors" : 2D numpy.array, (nr_vertices, BGRA)
            Decoded vertex colors where possible (uint8). Only the convex/
            concave curvature colors and direct RGB indices are decoded here;
            other index ranges (POI colors, functional LUT colors) are left
            as zeros -- use "vertex color indices" plus the relevant look-up
            table if you need those.
    ...
    with open(filename, 'rb') as f:
        ...
        # NOTE: BrainVoyager itself refuses to load files with a version
        # newer than what it knows about (see cpp: "if(FileVersion > 4.1f)").
        # We don't hard-fail here since older parsers may still work fine.
        ...
        # in pairs. The value 10000 codes the convex color of POI 1, the
        ...
        #
        # The reference BrainVoyager C++ loader (QDataStream, little endian)
        # reads this whole block as one raw int32 array ("SurfaceColor") and
        # does NOT decode it at load time -- decoding happens later wherever
        # it's needed. We mirror that: keep the raw indices around, and only
        # decode the cases we can (curvature colors, direct RGB) into
        # "vertex colors", without failing on the rest.

        n_vertices = header["Nr vertices"]

        # Raw per-vertex color index, as in BrainVoyager's own SurfaceColor.
        color_indices = np.zeros(n_vertices, dtype=np.int32)
        for i in range(n_vertices):
            color_indices[i], = struct.unpack('<i', f.read(4))
        mesh_data["vertex color indices"] = color_indices

        # Best-effort decode into BGRA, matching only the cases the header
        # comment (and the cpp loader's downstream rendering code) documents
        # unambiguously. Unhandled index ranges (POI colors 10000-10200,
        # functional LUT colors >=1000) are left as zeros; use
        # mesh_data["vertex color indices"] with the relevant look-up table
        # if you need those decoded too.
        vertex_colors = np.zeros((n_vertices, 4), dtype=np.uint8)
        for i in range(n_vertices):
            idx = int(color_indices[i])

            if idx >= 1056964608:  # direct RGB color packed into the index
                raw_bytes = idx.to_bytes(4, byteorder='little', signed=False)
                vertex_colors[i, 0] = raw_bytes[0]
                vertex_colors[i, 1] = raw_bytes[1]
                vertex_colors[i, 2] = raw_bytes[2]
                vertex_colors[i, 3] = raw_bytes[3]

            elif idx == 0:  # convex curvature color
            ...
            elif idx == 1:  # concave curvature color
            ...
            # else: POI color (10000-10200) or functional LUT color (>=1000)
            # -- left as zeros; needs the corresponding look-up table to
            # resolve to an actual color.
    ...
    with open(filename, 'rb') as f:
        ...
        """
            ...
            # TODO: Implement other indices too
                ...
                raise Exception("Bad vertex color index! Should be 0, 1 or >=1056964608.")
        """
        # ---------------------------------------------------------------------
        # Loop over nearest neighbor data for each vertex
        temp = []
...
def create_srf():
    ...
    mesh_data = {'vertices': np.array([], dtype=np.float32),
                 'vertex normals': np.array([], dtype=np.float32),
                 'vertex neighbors': list(list()),
                 'faces': np.array([], dtype=np.int32),
                 'Strip sequence': list(list()),
                 "vertex_colors":np.concatenate([175*np.ones((header['Nr vertices'],3)), np.ones((header['Nr vertices'],1))], axis=1)}
    return header, mesh_data
